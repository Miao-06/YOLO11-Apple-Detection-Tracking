...
# ============================================================
# 7.2 V3：整段视频 Tracklet 全局拼接
#
# V2 是在线决策：
#   当前看不到旧 Track -> 有可能新建 ID。
#
# V3 多走一步：
#   整段视频处理完后，再检查这些“断裂的短轨迹”，
#   如果它们在参考场景坐标中非常接近、尺寸也接近，
#   而且时间上没有明显冲突，就把它们合并成同一苹果。
#
# 这一步专门针对：
#   遮挡
#   横移
...
# 两个 tracklet 的 scene_center 距离，
# 除以它们典型框对角线后，小于这个值才允许拼接。
STITCH_SCENE_DISTANCE_FACTOR = 0.90

# 典型面积比例限制。
STITCH_MAX_AREA_RATIO = 2.20

# 如果两个 tracklet 在时间上同时存在，
# 基本不可能是同一个苹果，因此禁止合并。
STITCH_MAX_FRAME_OVERLAP = 0

# 时间间隔很长时，允许稍微放宽 scene 距离。
# 每 30 帧增加一点容忍，但有上限。
STITCH_GAP_RELAX_PER_30F = 0.06
STITCH_MAX_EXTRA_RELAX = 0.30

# 为避免“相邻两个苹果被误合并”，
# 最佳候选必须明显优于第二候选。
STITCH_BEST_SECOND_GAP = 0.20

# 最少出现多少帧的短轨迹才参与最终苹果集合。
# 非常短的 tentative 轨迹不参与。
STITCH_MIN_HITS = 2
...
def build_tracklet_summaries(
    ...
    for (
        ...
        if len(observations) < STITCH_MIN_HITS:
            continue
...
def frame_overlap_amount(
    summary1,
    summary2
):
    """两个 tracklet 时间区间重叠多少帧。"""

    start = max(
        summary1["start_frame"],
        summary2["start_frame"]
    )
...
def frame_gap_amount(
    summary1,
    summary2
):
    """两个不重叠 tracklet 之间间隔多少帧。"""
...
def tracklet_stitch_cost(
    ...
    overlap = frame_overlap_amount(
        summary1,
        summary2
    )

    if overlap > STITCH_MAX_FRAME_OVERLAP:
        return None
    ...
    distance = point_distance(
        summary1["scene_center"],
        summary2["scene_center"]
    )
    ...
    gap = frame_gap_amount(
        summary1,
        summary2
    )

    extra_relax = min(
        STITCH_MAX_EXTRA_RELAX,
        (
            gap
            /
            30.0
        ...
        *
        STITCH_GAP_RELAX_PER_30F
    )

    allowed_distance = (
        STITCH_SCENE_DISTANCE_FACTOR
        +
        extra_relax
    )
...
class UnionFind:
    """简单并查集，用来把多个断裂 track_id 合成 canonical apple id。"""
...
def stitch_tracklets(
    ...
    而是对每一条后出现的 tracklet：
        best 候选必须存在
        且 best 明显优于 second best
    """

    track_ids = sorted(
        summaries.keys(),
        key=lambda tid: (
            summaries[tid]["start_frame"],
            tid
        )
    )
    ...
    # --------------------------------------------------------
    # 生成原 track_id -> canonical apple id
    # --------------------------------------------------------
...
def process_video(
    ...
    # PASS 1
    #
    # YOLO + 相机运动 + V2 在线 Tracker
    #
    # 这里不直接写最终视频。
    # 先把每一帧 Track 状态和每条 tracklet 历史记录下来。
    # ========================================================
    ...
    # 每一帧保存“原始 Track ID + box”。
    frame_records = []


    # 每一个原始 Track 的长期 scene 历史。
    track_history = {}
    ...
    while True:
        ...
        for (
            track_id,
            track
        ) in tracker.tracks.items():

            # 只保存当前帧真正匹配到 detection 的 Track。
            # missed > 0 属于 TTL 残留，不作为新 observation。
            if track["missed"] != 0:
                continue
    ...
    print(
        f"[V3 全局拼接] "
        f"有效 tracklets: {len(summaries)}"
    )
    print(
        f"[V3 全局拼接] "
        f"合并断裂轨迹数: {merged_count}"
    )
    print(
        f"[V3 全局拼接] "
        f"V2 online total: {online_total}"
    )
    print(
        f"[V3 全局拼接] "
        f"V3 stitched total: {stitched_total}"
    )
    print("-" * 75)
    ...
    #
    # 重新读取原始视频。
    # 使用 canonical apple id 绘制最终视频。
    #
    # 这样 Total Apples 是经过“整段视频全局去重”后的结果。
    # ========================================================
    ...
    output_path = (
        OUTPUT_DIR
        /
        f"{video_path.stem}_apple_scene_tracker_v3.mp4"
    )
    ...
    # 到当前帧为止已经见过的 canonical apple IDs。
...
def main():
    ...
    print("=" * 75)
    print(
        "Fruit YOLO Apple Scene Tracker V3"
    )
    print(
        "Exp003 + Camera Motion + Tracklets + Global Stitching + Total Count"
    )
    print("=" * 75)
    ...
    print(
        f"V3 stitch scene factor = "
        f"{STITCH_SCENE_DISTANCE_FACTOR}"
    )

    print(
        f"V3 stitch max area ratio = "
        f"{STITCH_MAX_AREA_RATIO}"
    )

    print(
        f"V3 stitch best-second gap = "
        f"{STITCH_BEST_SECOND_GAP}"
    )
